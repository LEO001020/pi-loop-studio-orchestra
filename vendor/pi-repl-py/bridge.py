...
# JSON-lines transport is UTF-8 on every Windows code page, including when the
# workspace, connection file or executed code contains Chinese characters.
...
READY_TIMEOUT_MS = int(os.environ.get("PI_KERNEL_READY_SECONDS", "120")) * 1000
...
class Bridge:
    ...
    def start_kernel(self):
        # Use Jupyter's supported local TCP/HMAC transport. No AppContainer,
        # CurveZMQ monkeypatch, custom Windows kernel or watchdog is involved.
        runtime = os.environ["JUPYTER_RUNTIME_DIR"]
        os.makedirs(runtime, exist_ok=True)
        km = KernelManager(kernel_name="python3", transport="tcp", ip="127.0.0.1",
                           connection_file=os.path.join(runtime, "kernel-" + uuid.uuid4().hex + ".json"))
        try:
            km.kernel_spec.argv = [sys.executable, "-X", "utf8", "-m", "ipykernel_launcher", "-f", "{connection_file}"]
            km.start_kernel(cwd=os.getcwd())
        except NoSuchKernel:
            raise RuntimeError("ipykernel kernelspec is missing from the bundled Python environment")
        kc = km.client()
        kc.start_channels()
        try:
    ...
    def run_cell(self, code, emit_stream, payload_mime=None, emit_id=None):
        ...
        if os.environ.get("ASTRA_REPL_DIAGNOSTICS") == "1":
            print("JUPYTER_CELL_START", payload_mime or "user", file=sys.stderr, flush=True)
        kc = self.kc
        ...
        if os.environ.get("ASTRA_REPL_DIAGNOSTICS") == "1":
            print("JUPYTER_CELL_END", payload_mime or "user", status, file=sys.stderr, flush=True)
    ...
    def serve(self):
        ...
        try:
            ...
            import traceback
            self.emit({"type": "error", "message": "boot failed: %s" % e, "stack": traceback.format_exc()[-6000:]})
            sys.exit(1)
        while True:
            try:
